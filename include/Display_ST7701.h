/* 
The ST7701 is the LCD controller chip on the Waveshare ESP32-S3-LCD-2.8C board. Display_ST7701.h/.cpp contains all the low-level hardware code to drive it, specifically:

LCD initialization — sends the configuration commands to the ST7701 chip over SPI to set it up (resolution, color format, timing, etc.)
RGB panel setup — configures the ESP32-S3's built-in RGB LCD peripheral to stream pixel data to the display
Backlight control — Backlight_Init() and Set_Backlight() use PWM to control brightness
Pixel drawing — the LCD_Draw_Bitmap() function which LVGL calls to actually push rendered pixels to the screen

It's the bridge between LVGL (which just renders into a buffer) and the actual screen.
*/
...
#define ESP_PANEL_LCD_RGB_BOUNCE_BUF_SIZE         (20 * ESP_PANEL_LCD_HEIGHT)     // Bounce buffer size in bytes. This function is used to avoid screen drift.
