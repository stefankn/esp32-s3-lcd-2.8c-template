...
- The clock colon blinks every second via `lv_obj_set_style_text_opa()` toggling between `LV_OPA_COVER` and `LV_OPA_TRANSP` — avoids missing-glyph artifacts from using a space character with a limited-glyph font
...
- Bounce buffer is set to `20 × LCD_HEIGHT` (19.2 KB) to reduce DMA interrupt frequency and minimize display artifacts under WiFi load
